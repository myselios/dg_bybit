#!/bin/bash
# scripts/monitor_mainnet.sh
# Mainnet 실행 상태를 실시간으로 간단하게 보여주는 모니터링 도구

LOG_FILE="logs/mainnet_dry_run/mainnet_dry_run.log"
...
echo "================================================="
echo "🚀 Mainnet 모니터링 대시보드"
echo "================================================="
echo ""

# 로그 파일이 생성될 때까지 대기
while [ ! -f "$LOG_FILE" ]; do
    echo "⏳ Mainnet 시작 대기 중..."
    sleep 2
done

echo "✅ Mainnet 실행 감지! 실시간 모니터링 시작..."
echo ""

# 실시간 모니터링 (핵심 이벤트만 추출)
tail -f "$LOG_FILE" | while read line; do
    # 시작 메시지
    if echo "$line" | grep -q "Starting Mainnet Dry-Run"; then
        echo -e "${GREEN}[시작]${NC} Mainnet 실행 시작"
    fi
