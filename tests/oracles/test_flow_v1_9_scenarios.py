...
TODO (Phase 3):
3. test_degraded_60s_halt - DEGRADED 60초 후 HALT
4. test_rest_budget_tick_increase - REST Budget 초과 시 tick 주기 증가
5. test_size_overflow_excess_close - Position size overflow → 초과분 청산
6. test_partial_fill_immediate_stop - 부분체결 즉시 IN_POSITION + Stop (이미 존재)
"""
...
class TestFlowV19Scenarios(unittest.TestCase):
    ...
    def test_event_dedup_ignores_duplicate(self):
        ...
        Given:
          - 동일 order_id, type, timestamp 이벤트 2회
        ...
        FLOW 규칙:
        - dedup_key = f"{order_id}_{type}_{timestamp}"
        - 동일 key → 무시

        TODO: ExecutionEvent에 execution_id 필드 추가 후 정확한 dedup key 사용
        """
        # Given
