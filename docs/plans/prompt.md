```md
# IMPLEMENTATION PROMPT — Account Builder (Bybit Inverse) Bot Builder
Last Updated: 2026-01-19 (KST)
Status: FINAL (PRE-FLIGHT HARD GATES + SSOT + Evidence Required)
...
- 오라클은 `transition()`을 **직접 호출**하고,
  `(expected_state, expected_position, expected_pending, expected_intents)`를 정확히 assert 해야 한다.
- 오라클에서 EventRouter를 테스트하지 않는다. (EventRouter는 별도 unit test로 “위임만” 검증)

2) No Test-Defined Domain
- 테스트 코드에서 `Position/PendingOrder/ExecutionEvent/State`를 재정의하거나 별도 dataclass로 만들지 않는다.
- 반드시 `src/domain/state.py`, `src/domain/events.py`, `src/domain/intent.py`에 정의된 타입만 사용한다.
- 테스트 helper는 허용하지만 “도메인 구조 복제(동일 이름/동일 필드)”는 금지.

3) Single Transition Truth (SSOT)
...
- `docs/plans/task_plan.md`의 Repo Map과 실제 파일 경로가 다르면 Phase 0에서 정렬한다.
  - 선택지 A: 문서대로 코드 경로를 이동/정리
  - 선택지 B: 문서를 실제 경로에 맞게 수정
- 어느 쪽이든 “단일 진실 경로(SSOT 경로)”를 확정하기 전엔 다음 Phase로 넘어가지 않는다.
...
**[추가: 테스트 실행 표준(협상 불가)]**
- 테스트는 `python -m pip install -e .` 이후 `pytest`가 **PYTHONPATH 설정 없이** 동작해야 DONE.
- (예외를 만들고 싶다면 ADR로만 허용)

6) Doc Update is Part of the Work
- 기능/테스트 완료 시 `docs/plans/task_plan.md`를 즉시 업데이트한다.
  - Last Updated 갱신
...
**[추가: PRE-FLIGHT 완료 증거 제출 강제]**
- PRE-FLIGHT 완료 보고에는 Gate 1~5의 **명령어 출력(붙여넣기)** + **pytest 결과** + **변경된 파일 목록**을 반드시 포함한다.
- “응 했음” 금지. 증거가 없으면 PRE-FLIGHT DONE 불가.
...
## 역할
너는 Account Builder(Bybit Inverse) 트레이딩 봇의 구현자다.
...
- `transition()`은 **pure(무 I/O)** 이며, I/O는 executor에서만 수행한다.
- 변경이 정책/정의/단위(ADR Required)에 해당하면 작업을 중단하고 ADR 필요성을 보고한다.

---

## 지금 할 일
1) 1순위: PRE-FLIGHT를 완료한다.
   - (특히 오라클 placeholder 제거 + 도메인 재정의 제거 + 단일 전이 엔진 확정 + repo map 정렬)
2) PRE-FLIGHT가 DONE되면 `docs/plans/task_plan.md`에서 "가장 먼저 TODO인 Phase"를 선택한다(Phase 0부터).
3) 해당 Phase의 Deliverables/Conditions/Tests/DoD를 충족하는 최소 구현을 한다.
4) 먼저 오라클/유닛 테스트를 작성해 실패(RED)를 확인한 뒤,
   최소 코드로 통과(GREEN)시키고 리팩토링한다.
5) 완료되면 Progress Table에 Evidence(테스트 경로/구현 경로/가능하면 커밋 해시)를 기록하고 Last Updated를 갱신한다.
