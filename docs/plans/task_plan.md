...
Status: Phase 0+1+2 COMPLETE (Evidence 확보 + ADR-0007 적용) | Gate 1-8 ALL PASS | DoD Aligned + Evidence System 구축 완료 | SSOT 정렬 완료 | Phase 3 준비 완료
...
| 2 | ✅ DONE | **Tests**: test_ids.py (6 cases) + test_entry_allowed.py (9 cases) + test_sizing.py (8 cases) + test_liquidation_gate.py (8 cases) = **31 passed**. Total: **114 passed in 0.11s** (83 → 114). **Gate 7**: ALL PASS (Placeholder 0, Skip/Xfail 0, Assert 181, Domain 재정의 0, transition SSOT, EventRouter thin). **SSOT Alignment**: FLOW Section 2 (gates), 3.4 (sizing), 7.5 (liquidation) + Policy Section 5 (stage params), 10 (sizing). | **Domain**: [ids.py](../../src/domain/ids.py) (signal_id/orderLinkId validators, SHA1 축약, 36자 제한). **Application**: [entry_allowed.py](../../src/application/entry_allowed.py) (7 gates + one-way + cooldown + reject 이유코드, Policy Section 5 준수), [sizing.py](../../src/application/sizing.py) (contracts 계산 LONG/SHORT 정확한 공식 + margin feasibility + tick/lot 보정 + 재검증, FLOW Section 3.4 준수), [liquidation_gate.py](../../src/application/liquidation_gate.py) (liq distance 계산 + 동적 기준 stop × multiplier + min_absolute + fallback, FLOW Section 7.5 준수). **RED→GREEN**: test_ids.py (6 passed), test_entry_allowed.py (9 passed), test_sizing.py (8 passed), test_liquidation_gate.py (8 passed). | **Commit**: 8d1c0d8 (2026-01-23). **Phase 2 완료**. DoD 4개 항목 충족 (signal_id/orderLinkId + 7 gates + sizing + liquidation). **Entry Flow 구현 완료**. Phase 3 시작 가능. |
| 3 | TODO | - | - | - |
| 4 | TODO | - | - | - |
| 5 | TODO | - | - | - |
| 6 | TODO | - | - | - |
